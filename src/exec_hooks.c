 ...
 *  	- called with at least these parameters:
 *  		--start {"call": "strace", "pid": <pid of strace>}
 * - callback block:
...
int get_current_directory(pid_t pid, char *current_directory);
...
int append_to_buffer_data_escaped(char **buffer, int *current_offset, int *buffer_size, char *data);
int append_to_buffer(char **buffer, int *current_offset, int *buffer_size, const char *data);
int append_string_to_json(char **buffer, int *current_offset, int *buffer_size, const char *field_name, char *data);
...
void add_exec_hook(char *exec) {
	...
	realpath(exec, new_exec_hook->exec_path);
...
void init_exec_hooks(void) {
	...
	snprintf(api_json, sizeof(api_json) - 1, "{\"version\":\"%s\",\"syscalls\":[\"execve\",\"clone\"]}", api_version);

	send_data_to_exec_hooks(CALLBACK_API, api_json);

	char start_json[100];
	snprintf(start_json, sizeof(start_json) - 1, "{\"call\":\"strace\",\"pid\":%d}", getpid());
...
int append_to_buffer_data_escaped(char **buffer, int *current_offset, int *buffer_size, char *data) {
	...
	while (*read_ptr != '\0') {
		if (*current_offset + 4 >= *buffer_size) {
	...
	*write_ptr++ = '\0';
	return *current_offset - prev_offset;
}
...
int append_to_buffer(char **buffer, int *current_offset, int *buffer_size, const char *data) {
	...
	if (data_size + *current_offset >= *buffer_size) {
		int new_buffer_size = *buffer_size * 2;
		char *new_buffer = realloc(*buffer, new_buffer_size * sizeof(char));
		if (new_buffer == NULL) {
			return 0;
...
int append_string_to_json(char **buffer, int *current_offset, int *buffer_size, const char *field_name, char *string) {
...
int append_execve_info_to_json(struct execve_info *syscall_info, char **buffer, int *current_offset, int *buffer_size) {
	int success = append_string_to_json(buffer, current_offset, buffer_size, "exec_path", syscall_info->exec_path);
	if (success < 0) {
		return -1;
	}
	success = append_string_to_json(buffer, current_offset, buffer_size, "current_directory", syscall_info->current_directory);
	if (success < 0) {
		return -1;
	}
	success = append_string_array_to_json(buffer, current_offset, buffer_size, "argv", syscall_info->argv, syscall_info->argv_length);
	if (success < 0) {
		return -1;
	}
	return 0;
...
char *build_json(struct syscall_info *syscall_info) {
	...
	current_offset = snprintf(json_buffer, buffer_size - 1, "{\"syscall\":\"%s\",\"pid\":%d,\"ppid\":%d", get_syscall_name(syscall_info->type), syscall_info->pid, syscall_info->ppid);
...
pid_t get_ppid(pid_t pid) {
	...
	snprintf(process_information_path, path_size, "/proc/%u/status", pid);
...
int get_current_directory(pid_t pid, char *current_directory) {
	...
	if (snprintf(buffer, buffer_size - 1, "/proc/%d/cwd", pid) < (int)sizeof(buffer)) {
		ssize_t length = readlink(buffer, current_directory, PATH_MAX - 1);
...
void execve_exec_hook(struct tcb *tcp, const unsigned int index) {
	...
	int current_directory_length = get_current_directory(syscall_info.syscall.pid, syscall_info.current_directory);
